...
public final class NiceScoreStrategy implements DeliveryStrategy {
    @Override
    public ArrayList<ChildOutput> makeDelivery(final SantaClaus santa) {
        ...
        // after score calculation, sort children in descending order
        // based on the score
        Comparators.SortChildrenByNiceScoreCalculated sortNiceScore =
                new Comparators.SortChildrenByNiceScoreCalculated();
        santa.getChildren().sort(sortNiceScore);

        // make a list for children who don't receive a gift
        ArrayList<Child> childWithNoGift = new ArrayList<>();

        // for every child, give gifts if the child has enough budget
        for (int i = 0; i < santa.getChildren().size(); ++i) {
            // child's budget
